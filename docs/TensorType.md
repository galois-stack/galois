...
张量(Tensor)可以理解为标量和向量概念的延伸，它是数学中用来描述多维关系的一种对象。在机器学习里，通常把多维的数据叫作张量，并用秩来表示它有多少个维度。图-1简单展示了不同维度的张量：
...
```c++
auto ir_value_type = ir::f32; // 张量的value_type
Eigen::VectorXi64 shape(2); // 张量的纬度
shape[0] = 16; // 尺寸
shape[1] = 16;
...
我们可以看出, 当张量的尺寸很大时, 只有一个纬度的相邻两个元素是内存连系的, 其他纬度会存在非常大的跨度.
这是非常不利于我们进行矩阵乘法, 卷积等存在多个纬度领域关系的计算. 我们必须寻求更为合适的张量布局, 而希尔伯特曲线给予
了我们很大的启发.
...
希尔伯特曲线的核心特性是局部连续性，在多个纬度上相邻的点在曲线上也接近，这种内存排布远优于单纯的行优先或列优先布局.

### 自相似

希尔伯特曲线具有分形结构，表现出自相似性, 局部形状与整体形状相似, 这种性质通过递归生成实现.
**这意味着我们针对该数据结构的算法(规则)也可以递归实现**. (这个结论凭经验得出)
