...
void BM_Full(benchmark::State &state) {
    ...
    auto ir_operator =
                ir_builder->CreateOperatorByCreator(ir_full_creator, {ir_value_type});
    ...
    auto full_fun = reinterpret_cast<float* (*)(float *)>(
        prajna_compiler->GetSymbolValue("::" + ir_full_creator->fullname));
