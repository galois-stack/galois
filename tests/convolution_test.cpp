...
void convolution1d_reference(float *output, const float *input, const float *weight, int input_c,
                             int input_w, int output_c, int output_w, int kernel_w,
                             int stride_w = 1) {
...
void convolution2d(float *output, float *input, float *weight, int rows_act, int rows_weight,
...
void convolution3d_reference(float *output, const float *input, const float *weight, int input_c,
                             int input_d, int input_h, int input_w, int output_c, int output_d,
                             int output_h, int output_w, int kernel_d, int kernel_h, int kernel_w,
                        ...
                        for (int kd = 0; kd < kernel_d; ++kd) {
                            for (int kh = 0; kh < kernel_h; ++kh) {
                                for (int kw = 0; kw < kernel_w; ++kw) {
                                    ...
                                    // Check bounds
                                    if (id >= 0 && id < input_d && ih >= 0 && ih < input_h &&
                                        iw >= 0 && iw < input_w) {
                                        ...
                                        int weight_idx =
                                            oc * input_c * kernel_d * kernel_h * kernel_w +
...
TEST(GaloisTests, TestConvolution1D) {
    ...
    convolution1d_reference(expected.data(), input.data(), weight.data(), input_c, input_w,
                            output_c, output_w, kernel_w, stride);
...
TEST(GaloisTests, TestConvolution2D_3x3) {
    ...
    auto ir_weight_type =
        ir::f32->Tile(output_channels, input_channels, rows_weight, cols_weight);  // [1, 1, 3, 3]
    auto ir_builder = ir::Builder::Create();
    auto ir_operator = ir_builder->CreateOperatorByCreator<op::ConvolutionNDCreator<2>>(
        {ir_act_type, ir_weight_type}, std::array<int, 2>{stride, stride});
    ...
    convolution2d(output.data(), input_2d.data(), weight_2d.data(), rows_act, rows_weight, stride,
                  padding);
...
TEST(GaloisTests, TestConvolution3D_Volume) {
    ...
    convolution3d_reference(expected.data(), input.data(), weight.data(), input_c, input_d, input_h,
                            input_w, output_c, output_d, output_h, output_w, kernel_d, kernel_h,
                            kernel_w, strides[0], strides[1], strides[2]);
