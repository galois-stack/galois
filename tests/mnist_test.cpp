...
TEST(GaloisTests, TestMnistCNN) {
    ...
    auto [ir_operator, scope] = ir_builder->CreateOperator(
        ir::OperatorType::Create({ir::f32->Tile(1, 28, 28)}, ir::f32->Tile(10)), "mnist_cnn");
    ...
    auto ir_fc1_weights =
        ir_builder->Create<ir::io::LoadBinary>(ir_fc1_type, "tests/models/mnist/fc1_weight.bin");
    auto ir_fc2_weights =
        ir_builder->Create<ir::io::LoadBinary>(ir_fc2_type, "tests/models/mnist/fc2_weight.bin");

    // Conv1 + ReLU: [28,28,1] -> [12,12,32] (kernel=5, stride=2, (28-5)/2+1=12)
    auto ir_conv1 = ir_builder->ExpressCreator<op::ConvolutionNDCreator<2>>(
        {ir_input, ir_conv1_weights}, std::array<int, 2>{2, 2});
    auto ir_relu1 = ir_builder->ExpressCreator<op::ReluCreator>({ir_conv1});

    // Conv2 + ReLU: [12,12,32] -> [4,4,64] (kernel=5, stride=2, (12-5)/2+1=4)
    auto ir_conv2 = ir_builder->ExpressCreator<op::ConvolutionNDCreator<2>>(
        {ir_relu1, ir_conv2_weights}, std::array<int, 2>{2, 2});
    ...
    // FC1 + ReLU
    auto ir_fc1 =
        ir_builder->ExpressCreator<op::MatrixMultiplyCreator>({ir_flatten, ir_fc1_weights});
