...
class FillWithParamTest
   ...
   public:
    void SetUp() override {
        ...
        this->fill_fun = jit_engine->EmitOperatorSymbol<void (*)(void *, void *)>(ir_operator);

        this->items = ir_input_type->Size();
        this->input_vec.resize(this->items * ir_data_type->bytes);
    }
    ...
    void TearDown() override {
        ...
        fmt::print("Galois cost time: {}ns, galois flops: {:.04f}gops, bandwidth: {:.04f}\n",
                   galois_cost_time, items / galois_cost_time, bandwidth);
    }
...
TEST_P(FillWithParamTest, TestFillWithParamTest) {
    ...
    fill_fun(this->input_vec.data(), &this->fill_value);
    ...
    if (auto float_type = std::dynamic_pointer_cast<ir::FloatType>(this->ir_data_type)) {
        if (float_type->bits == 32) {
            float *float_ptr = reinterpret_cast<float *>(input_vec.data());
...
INSTANTIATE_TEST_SUITE_P(Scalar, FillWithParamTest,
                         testing::Combine(testing::Values(ir::f32),  // 数据类型
...
INSTANTIATE_TEST_SUITE_P(FillWithParamTest, FillWithParamTest,
                         testing::Combine(testing::Values(ir::f32),  // 数据类型
                                          testing::Values(std::vector<int64_t>{512, 512},
                                                          std::vector<int64_t>{512, 512, 3},
                                                          std::vector<int64_t>{512, 512, 3, 4}),
                                          testing::Values(1.0f, 9.0f, 42.0f)),  // fill_value
                         galois::test::PrintTestNameWithVector);
