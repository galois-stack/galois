...
void print_torch_tensor(const char* name, pybind11::object& tensor) {
    pybind11::array_t<float> arr =
        tensor.attr("cpu")().attr("numpy")().cast<pybind11::array_t<float>>();
...
void calculate_error(void* mem_a, void* mem_b, void* mem_c, int normalize_m, int normalize_k,
    ...
    pybind11::array_t<float> arr_c_tensor =
        tensor_c.attr("cpu")().attr("numpy")().cast<pybind11::array_t<float>>();
    ...
    if (shape[0] != normalize_m || shape[1] != normalize_n) {
        printf("Error: tensor_c shape mismatch! Expected (%d, %d), got (%lld, %lld)\n", normalize_m,
               normalize_n, static_cast<long long>(shape[0]), static_cast<long long>(shape[1]));
        return;
    }
