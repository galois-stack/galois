...
# 调试：检查find命令是否找到文件，仅搜索galois和tests目录
echo "正在查找galois和tests目录中的.cpp、.h、.hpp、.cxx文件..."
FOUND_FILES=$(find galois tests \( -name "*.cpp" -o -name "*.h" -o -name "*.hpp" -o -name "*.cxx" \) 2>/dev/null)
if [ -z "$FOUND_FILES" ]; then
    echo "在galois和tests目录中未找到任何.cpp、.h、.hpp、.cxx文件，跳过clang-format检查。"
    exit 0
else
    echo "找到以下文件："
    # echo "$FOUND_FILES"
...
# 运行clang-format检查，捕获stdout和stderr，仅检查galois和tests目录
echo "运行clang-format检查..."
find galois tests \( -name "*.cpp" -o -name "*.h" -o -name "*.hpp" -o -name "*.cxx" \) -exec clang-format --dry-run --verbose {} \; >> "$TEMP_FILE" 2>&1
