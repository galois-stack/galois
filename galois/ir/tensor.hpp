...
class Indexing : public Instruction {
   public:
    static std::shared_ptr<Indexing> Create(std::shared_ptr<Tensor> ir_tensor, std::vector<std::shared_ptr<Tensor>> ir_indices) {
    ...
    void Index(int64_t i, std::shared_ptr<ir::Tensor> ir_index) { this->SetOperand(1 + i, ir_index); }
