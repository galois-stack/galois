...
class ReduceProdCreator : public op::Creator {
   public:
    static std::shared_ptr<ReduceProdCreator> Create() {
        auto self = std::make_shared<ReduceProdCreator>();
    ...
    void Express(std::vector<std::shared_ptr<ir::Tensor>> ir_inputs,
        ...
        ir_builder->ExpressCreator<op::FillCreator>(
            {ir_re, ir_builder->GetConstant(ir_re_type->DataType(), 1.0)});
    ...
    void ExpressInline(std::shared_ptr<ir::Tensor> ir_input, std::shared_ptr<ir::Tensor> ir_re,
        ...
        if (ir_input->type->IsScalar()) {
            auto ir_add = ir_builder->Mul(ir_input, ir_re);
